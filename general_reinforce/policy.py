...
class Policy(object):
    ...
    def __init__(self, config, env_info, sess=None):
        ...
        self._env_info = AttrDict(env_info)
        self._config = config
        self._build_model()
        self._set_loss()
    ...
    def _build_model(self):
        ...
        if self._env_info.is_continuous_observ:
            # Continuous observation
            shape = self._env_info.observ_shape
            self.observ = tf.placeholder(tf.float32, shape, name='observ')
        else:
            # Discrete observation
            shape = self._env_info.observ_shape
            self.observ = tf.placeholder(tf.int32, shape, name='observ')
        
        if self._env_info.is_continuous_action:
            # Continuous action
            shape = self._env_info.action_shape
            action_n = self._env_info.action_n
            self.action = tf.placeholder(tf.float32, shape, name='action')
        else:
            # Discrete action
            action_n = self._env_info.action_n
        ...
        x = tf.layers.dense(self.observ, action_n, use_bias=use_bias,
    ...
    def _set_loss(self):
        ...
        if self._env_info.is_continuous_action:
            raise NotImplementedError('Never implement Continuous loss.')
        else:
            # Discrete action
    ...
    def compute_action(self, observ, training=True):
        ...
        Returns:
            Action
        """
        assert observ.shape == (1, self._env_info.observ_shape[1])
        if self._env_info.is_continuous_action:
            # Continuous action
            action = self._sess.run(self.logits, feed_dict={self.observ: observ})
            assert isinstance(action, np.float64)
            raise NotImplementedError('Clip the action.')
        else:
            # Discrete action
...
class ValueFunction(object):
    ...
    def __init__(self, config, env_info, sess=None):
        ...
        """
        self._sess = sess
        # TODO: how to corresponding to ray
        self._env_info = AttrDict(env_info)
        self._config = config
        self._build_model()
        self._set_loss()

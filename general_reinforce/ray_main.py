...
from general_reinforce.agent import REINFORCE
from general_reinforce.policy import RemotePolicy
from general_reinforce.utils import plot_agent_stats
...
def main(_):
    ...
    env_info = get_env_info(gym.make(config.env_name))
    # Define Agent that train with REINFORCE algorithm.
    agent = REINFORCE(config, env_info)
    
    config_id = ray.put(config)
    env_info_id = ray.put(env_info)
    # env_info_id = ray.put(env_info)
    remote_policies = [RemotePolicy.remote(config_id, env_info_id) for _ in range(5)]
